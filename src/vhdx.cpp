...
constexpr size_t ONE_MEGABYTES { 1024 * 1024 };
constexpr size_t BLOCK_SIZE { USHRT_MAX + 1 };  // 65536
constexpr char BLOC_64KB[BLOCK_SIZE] { 0 };
constexpr size_t MIN_FILESIZE { 4 * ONE_MEGABYTES };

struct VHDX_REGION_TABLE_HEADER_ENTRIES {           // 65536 octets
    struct VHDX_REGION_TABLE_HEADER header;         // 16 octets
    struct VHDX_REGION_TABLE_ENTRY entries[2047];   // 32 octets * 2047 = 65504
    uint8_t alignment[16] { 0 };                    // 16 octets
};

const size_t VHDX_REGION_TABLE_HEADER_ENTRIES_SIZE { sizeof(struct VHDX_REGION_TABLE_HEADER_ENTRIES) };
...
VHDXOperationResult createVHDX(const std::filesystem::path& fileName, const size_t& fileSize) {
    VHDXOperationResult returnedValue;

    if (std::filesystem::exists(fileName)) {
        std::stringstream ss;
        ss << fileName << " already exists";
        returnedValue.error = ss.str();
        return returnedValue;
    }

    if (fileSize < MIN_FILESIZE) {
        std::stringstream ss;
        ss << "File size is too small (minimum size is : " << MIN_FILESIZE << " bytes)";
        returnedValue.error = ss.str();
        return returnedValue;
    }
    ...
    // Write "vhdxfile"         [0-65535]           ->  [0x00000000-0x0000FFFF]
    struct VHDX_FILE_IDENTIFIER sVHDXFileIdentifier;
    // std::cerr<<"Writing VHDX_FILE_IDENTIFIER (\"vhdxfile\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000010000]"<<std::endl;
    ...
    // Write "head" 1           [65536-131071]      ->  [0x00010000-0x0001FFFF]
    ...
    // std::cerr<<"Writing VHDX_HEADER (\"head\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000020000]"<<std::endl;
    ...
    // Write "head" 2           [131072-196607]     ->  [0x00020000-0x0002FFFF]
    ...
    // std::cerr<<"Writing VHDX_HEADER (\"head\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000030000]"<<std::endl;
    ...
    struct VHDX_REGION_TABLE_HEADER_ENTRIES sVHDXRegion1;
    sVHDXRegion1.header.EntryCount = 0x000000002;

    sVHDXRegion1.entries[0].Guid = { 0x2DC27766, 0xF623, 0x4200, { 0x9D, 0x64, 0x11, 0x5E, 0x9B, 0xFD, 0x4A, 0x08}};
    sVHDXRegion1.entries[0].FileOffset = 0x0000000000300000;
    sVHDXRegion1.entries[0].Length = 0x00100000;
    sVHDXRegion1.entries[0].Required = true;

    sVHDXRegion1.entries[1].Guid = { 0x8B7CA206, 0x4790, 0x4B9A, { 0xB8, 0xFE, 0x57, 0x5F, 0x05, 0x0F, 0x88, 0x6E}};
    sVHDXRegion1.entries[1].FileOffset = 0x0000000000200000;
    sVHDXRegion1.entries[1].Length = 0x00100000;
    sVHDXRegion1.entries[1].Required = true;

    sVHDXRegion1.header.Checksum = crc32_checksum(&sVHDXRegion1, VHDX_REGION_TABLE_HEADER_ENTRIES_SIZE);

    // std::cerr<<"Writing VHDX_REGION_TABLE (\"regi\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000040000]"<<std::endl;
    writtenBytes += writeData(ofs, reinterpret_cast<char*>(&sVHDXRegion1), VHDX_REGION_TABLE_HEADER_ENTRIES_SIZE);

    // std::cerr<<"Writing VHDX_REGION_TABLE (\"regi\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000050000]"<<std::endl;
    writtenBytes += writeData(ofs, reinterpret_cast<char*>(&sVHDXRegion1), VHDX_REGION_TABLE_HEADER_ENTRIES_SIZE);
     ...
     *************************************************************************/
    // Write "loge" 1           [1048576-1052671]   ->  [0x00100000-0x00100FFF]
    ...
    // std::cerr<<"Writing VHDX_LOG_ENTRY_HEADER (\"loge\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-0x0000000000101000]"<<std::endl;
    ...
    // Write "loge" 2           [1048576-1052735]   ->  [0x00101000-0x0010103F]
    struct VHDX_LOG_ENTRY_HEADER sVHDXLogEntryHeader2;
    sVHDXLogEntryHeader2.EntryLength = 0x00002000;    // Next entry
    sVHDXLogEntryHeader2.Tail = 0x00001000;           // Current Entry
    sVHDXLogEntryHeader2.SequenceNumber = 0x74DE700C4F98FA7C;
    sVHDXLogEntryHeader2.DescriptorCount = 0x0001;
    sVHDXLogEntryHeader2.Reserved = 0x0000;
    sVHDXLogEntryHeader2.LogGuid = { 0xB686CA6C, 0xFF05, 0x4673, { 0x9B, 0xAE, 0x8A, 0x29, 0xA8, 0x25, 0x98, 0xDB } };
    sVHDXLogEntryHeader2.FlushedFileOffset = 0x0000000000400000;
    sVHDXLogEntryHeader2.LastFileOffset = 0x0000000000400000;
    ...
    char* tmp = (char*)malloc(ONE_MEGABYTES);
    memset(tmp, 0x00, ONE_MEGABYTES);
    memcpy(&tmp[0],                             /* 0x1000 */
           &sVHDXLogEntryHeader2,
           VHDX_LOG_ENTRY_HEADER_SIZE);
    memcpy(&tmp[VHDX_LOG_ENTRY_HEADER_SIZE],    /* 0x1040 */
           &sVHDXLogDataDescriptor,
           VHDX_LOG_DATA_DESCRIPTOR_SIZE);
    memcpy(&tmp[sVHDXLogEntryHeader2.Tail],     /* 0x1000 */
           &sVHDXLogDatas[0],
           16 * VHDX_LOG_DATA_SECTOR_SIZE);

    sVHDXLogEntryHeader2.Checksum = crc32_checksum(tmp, sVHDXLogEntryHeader2.EntryLength);
    free(tmp);

    // std::cerr<<"Writing VHDX_LOG_ENTRY_HEADER (\"loge\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-"<<(writtenBytes + VHDX_LOG_ENTRY_HEADER_SIZE)<<"]"<<std::endl;
    writtenBytes += writeData(ofs, reinterpret_cast<char*>(&sVHDXLogEntryHeader2), VHDX_LOG_ENTRY_HEADER_SIZE);

    // std::cerr<<"Writing VHDX_LOG_DATA_DESCRIPTOR (\"desc\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-"<<(writtenBytes + VHDX_LOG_DATA_DESCRIPTOR_SIZE)<<"]"<<std::endl;
    writtenBytes += writeData(ofs, reinterpret_cast<char*>(&sVHDXLogDataDescriptor), VHDX_LOG_DATA_DESCRIPTOR_SIZE);

    writtenBytes += writeBlankData(ofs, 0x00102000 - writtenBytes);

    // std::cerr<<"Writing VHDX_LOG_DATA_SECTOR (\"data\") [0x"<<std::hex<<std::setw(16)<<std::setfill('0')<<writtenBytes<<"-"<<(writtenBytes + 16 * VHDX_LOG_DATA_SECTOR_SIZE)<<"]"<<std::endl;
    ...
    // std::cerr<<"Writing VHDX_METADATA_TABLE_HEADER (\"metadata\")"<<std::endl;
    ...
    sVHDXMetadataVirtualDiskSize.VirtualDiskSize = fileSize;
